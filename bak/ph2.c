...
int keys[NKEYS];
int nthread = 1;
pthread_mutex_t lock_b[NBUCKET];
...
static void insert(int key, int value, struct entry **p,
		           struct entry *n) {
...
int main(int argc, char *argv[]) {
	...
	for(int i = 0; i<NBUCKET; i++) {
		pthread_mutex_init(&lock_b[i],NULL);
	}
	//
	// first the puts
	//
